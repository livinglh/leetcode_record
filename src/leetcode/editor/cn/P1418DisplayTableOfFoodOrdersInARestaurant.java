...
class Solution {
    public List<List<String>> displayTable(List<List<String>> orders) {
        ...
        Set<String> sheet_0 = new HashSet<>();
        TreeMap<String, Map<String,Integer>> tablemap = new TreeMap<>(new Comparator<String>() {
            @Override
        ...
        for(List<String> order : orders){
            ...
            sheet_0.add(foodName);
            if(tablemap.containsKey(tableId)){
                Map<String, Integer> foodCountMap = tablemap.get(tableId);
                ...
                tablemap.put(tableId, foodCountMap);
            }
        }

        // 构造菜名
        List<String> food_name = new ArrayList<>(sheet_0);
        Collections.sort(food_name);

        // 构造每桌（每一行）数据
        for(String tableID : tablemap.keySet()){
            List<String> new_data = new ArrayList<>();
            new_data.add(tableID);
            Map<String, Integer> foodmap = tablemap.get(tableID);
            for(String foodname : food_name){
                new_data.add(String.valueOf(foodmap.getOrDefault(foodname,0)));
            }
            res.add(new_data);
        }

        // 完善表头
        food_name.add(0, "Table");
        res.add(0,food_name);

...
//java:把字符串转换成整数
public class P面试题67BaZiFuChuanZhuanHuanChengZhengShuLcof{
    public static void main(String[] args) {
        Solution solution = new P面试题67BaZiFuChuanZhuanHuanChengZhengShuLcof().new Solution();
    }
...
class Solution {
    public int strToInt(String str) {
        ...
        if(start == str.length() || start == -1) return 0;
        for (int i = start; i < str.length(); i++) {
            char c = str.charAt(i);
            if(c >= '0' && c <= '9') sum = sum * 10 + (str.charAt(i) - '0');
            else break;
        }
